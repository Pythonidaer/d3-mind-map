...
export const nodes = [
    ...
    { id: 'functionalComponents', label: 'Functional\nComponents', definition: 'Stateless or stateful components written as functions.', shape: 'rect', color: COLORS.mintGreen },
    { id: 'jsx', label: 'JSX Syntax', definition: 'A syntax extension that looks like HTML and is used to describe UI structure.', shape: 'rect', color: COLORS.mintGreen },
  
    // Grandchildren: Hooks
    { id: 'useState', label: 'useState', definition: 'Hook to manage local state in a functional component.', shape: 'rect', color: COLORS.mintGreen },
    { id: 'useEffect', label: 'useEffect', definition: 'Hook to perform side effects such as data fetching or subscriptions.', shape: 'rect', color: COLORS.mintGreen },
    { id: 'useContext', label: 'useContext', definition: 'Hook to consume values from React context without prop drilling.', shape: 'rect', color: COLORS.mintGreen },
    { id: 'useRef', label: 'useRef', definition: 'Hook for referencing DOM elements or storing persistent mutable values.', shape: 'rect', color: COLORS.mintGreen },
    { id: 'useMemo', label: 'useMemo', definition: 'Hook that memoizes computed values to avoid expensive calculations on re-render.', shape: 'rect', color: COLORS.mintGreen },
    { id: 'useCallback', label: 'useCallback', definition: 'Hook that memoizes functions to prevent unnecessary re-renders.', shape: 'rect', color: COLORS.mintGreen },
  
    // Grandchildren: State & Props
    { id: 'propDrilling', label: 'Prop\nDrilling', definition: 'Passing data through multiple levels of components.', shape: 'rect', color: COLORS.mintGreen },
    { id: 'liftingState', label: 'Lifting\nState Up', definition: 'Moving state to the nearest common ancestor to share between children.', shape: 'rect', color: COLORS.mintGreen },
    { id: 'controlledProps', label: 'Controlled\nProps', definition: 'Props used to control form inputs or child behavior.', shape: 'rect', color: COLORS.mintGreen },
  
    // Grandchildren: Rendering Logic
    { id: 'conditionalRendering', label: 'Conditional\nRendering', definition: 'Show UI based on certain conditions using if, &&, ternary, etc.', shape: 'rect', color: COLORS.mintGreen },
    { id: 'listRendering', label: 'List\nRendering', definition: 'Render repeated components with array.map and unique keys.', shape: 'rect', color: COLORS.mintGreen },
  
    // Grandchildren: Composition
    { id: 'childrenProp', label: 'Children\nProp', definition: 'Special prop to allow components to nest and render passed content.', shape: 'rect', color: COLORS.mintGreen },
    { id: 'customHooks', label: 'Custom\nHooks', definition: 'Encapsulate reusable logic using shared hooks.', shape: 'rect', color: COLORS.mintGreen },
    { id: 'compoundComponents', label: 'Compound\nComponents', definition: 'Pattern to share state between parent and nested components via context.', shape: 'rect', color: COLORS.mintGreen },
  
    // Grandchildren: Forms
    { id: 'formState', label: 'Form\nState', definition: 'Track and update input values using useState.', shape: 'rect', color: COLORS.mintGreen },
    { id: 'validation', label: 'Validation', definition: 'Ensure form input meets required conditions before submit.', shape: 'rect', color: COLORS.mintGreen },
  
    // Grandchildren: Event Handling
    { id: 'onClick', label: 'onClick /\nonChange', definition: 'React’s SyntheticEvent system for handling user interaction.', shape: 'rect', color: COLORS.mintGreen },
    { id: 'preventDefault', label: 'preventDefault()', definition: 'Prevents default browser behavior such as page reload on form submit.', shape: 'rect', color: COLORS.mintGreen },
  
    // Grandchildren: Cons
    { id: 'boilerplate', label: 'Boilerplate', definition: 'May require lots of setup for routing, state management, and build tools.', shape: 'rect', color: COLORS.lighterRed },
    { id: 'learningCurve', label: 'Learning\nCurve', definition: 'JSX, hooks, and the component lifecycle can be challenging for beginners.', shape: 'rect', color: COLORS.lighterRed },
    { id: 'overheadSimpleSites', label: 'Overhead for\nSimple Sites', definition: 'React might be overkill for static or low-interactivity sites.', shape: 'rect', color: COLORS.lighterRed },
  
    // Grandchildren: Anti-Patterns
    { id: 'propDrillingAP', label: 'Prop\nDrilling', definition: 'Excessively passing props through many layers. Consider context.', shape: 'rect', color: COLORS.lighterRed },
    { id: 'overuseUseEffect', label: 'Overusing\nuseEffect', definition: 'Using useEffect for things that don’t need it, causing unnecessary renders.', shape: 'rect', color: COLORS.lighterRed },
    { id: 'anonymousFunctionsJSX', label: 'Anonymous\nFunctions in JSX', definition: 'Inline arrow functions in render may cause re-renders and performance issues.', shape: 'rect', color: COLORS.lighterRed },
    { id: 'mutatingStateDirectly', label: 'Mutating\nState Directly', definition: 'Modify state using setters instead of direct mutation to avoid stale state bugs.', shape: 'rect', color: COLORS.lighterRed },
    { id: 'missingKeys', label: 'Missing\nKeys in Lists', definition: 'Lack of stable unique keys in list rendering affects diffing performance.', shape: 'rect', color: COLORS.lighterRed },
    {
        id: 'errorHandling',
        label: 'Error\nBoundaries',
